...
func _regenerate_file_structure():
	...
	# add externals in future. this might not work on export but something must
	var globalized = ProjectSettings.globalize_path("res://")
	...
	if dir:
		...
		while file_name != "":
			if dir.current_is_dir():
				...
				_add_info(globalized + "/" + file_name, known_scores)
